...
distributions = {
  :precise64 => {
    ...
    :run_list => %w| apt build-essential vim java monit elasticsearch elasticsearch142::plugins elasticsearch142::proxy elasticsearch142::aws elasticsearch142::data elasticsearch142::monit |,
    :ip       => '33.33.33.10',
    :primary  => true,
    :node     => {
      :elasticsearch => {
  ...
  :centos6 => {
    ...
    :run_list => %w| yum::epel build-essential vim java elasticsearch elasticsearch142::proxy elasticsearch142::data |,
    :ip       => '33.33.33.11',
    :primary  => false,
    :node     => {
      :elasticsearch => {
  ...
  distributions.each_pair do |name, options|
    ...
    config.vm.define name, :options => options[:primary] do |box_config|
      ...
      box_config.vm.provision :chef_solo do |chef|
        ...
        chef.run_list << 'elasticsearch142::test' if ENV['TEST']
