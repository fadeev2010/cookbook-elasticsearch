module Extensions
  ...
  #
  # The credentials for accessing AWS API are loaded from `node.elasticsearch142.cloud`.
    ...
    ruby_block "Create EBS volume on #{device} (size: #{params[:ebs][:size]}GB)" do
      ...
      block do
        ...
        unless server.volumes.map(&:device).include?(ebs_device)
          ...
          aws.tags.new(:key => "ClusterName", :value => node.elasticsearch142[:cluster][:name], :resource_id => volume.id, :resource_type => "volume").save
